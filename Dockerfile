...
RUN export TMPDIR="/tmp/pip_tmp"
RUN ls /code/BLAST_CT_V2_4AIDE \
    && cp /code/BLAST_CT_V2_4AIDE/requirements.txt /tmp/pip_tmp/
RUN pip3 --disable-pip-version-check --no-cache-dir install -r /tmp/pip_tmp/requirements.txt \
    && rm -rf /tmp/pip_tmp
